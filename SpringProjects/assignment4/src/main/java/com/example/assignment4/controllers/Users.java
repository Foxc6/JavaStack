...
public class Users {
	...
	public String getUsers(HttpSession session, Model model,
			@ModelAttribute("potentialFriend") PotentialFriends potentialFriend) {
	...
	public String showCurrentUser(Model model, HttpSession session,
			@ModelAttribute("potentialFriend") PotentialFriends potentialFriend,
			@ModelAttribute("friend") Friends friend) {
		// First make sure that a user is logged in to view page
		if (session.getAttribute("user_id") == null) {
			return "redirect:/";
		}
	...
	public String sendRequest(@Valid @ModelAttribute("potentialFriend") PotentialFriends potentialFriend,
			HttpSession session) {
...
//		userService.destroyPendingFriend(friend.getId());
		return "redirect:/professionalProfile";
	}
	
	@RequestMapping("/ignore")
	public String ignore(@Valid @ModelAttribute("friend") Friends friend, HttpSession session) {
		System.out.println("Wish this would delete a friend request");
		return "redirect:/professionalProfile";
	}
